...
class TFIDFMatrix(Corpus):
    def __init__(self, files_dir):
        super().__init__(files_dir=files_dir)
        self.matrix_filepath = os.path.join(self.files_dir, 'matrix_tfidf.npz')
        self.vectorizer_filepath = os.path.join(self.files_dir, 'tfidf_vectorizer.pkl')
        try:
            ...
            print('This might take a while...')
            self.matrix, self.tfidf_vectorizer = self.get_matrix()

    def get_matrix(self):
        """Initiates vectorizer and creates matrix"""
        vectorizer = TfidfVectorizer()
        matrix = vectorizer.fit_transform(self.clean_corpus)
    ...
    def query_vect(self, query):
        ...
        vec_q = self.tfidf_vectorizer.transform([clean_q])
        return vec_q
